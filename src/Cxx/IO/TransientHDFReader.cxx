...
void Animate(vtkObject* caller, unsigned long eid, void* clientdata, void* calldata)
{
  ...
  vtkHDFReader* reader = vtkHDFReader::SafeDownCast(static_cast<vtkObjectBase*>(clientdata));
  reader->SetStep(
    (reader->GetStep() == reader->GetNumberOfSteps() - 1) ? 0 : reader->GetStep() + 1);
...
int main(int ac, char** av)
{
  if (ac != 2)
  {
    std::cout << "Usage: " << av[0] << " filename.vtkhdf eg. warping_spheres.vtkhdf" << std::endl;
    return EXIT_FAILURE;
  }
